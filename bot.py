...
Token = '1728092987:AAFxCfauB6FQu94FSyosCoq2YjMaPAZdKGY'
...
def busInfo(update,context):
    ...
    for bus in bus_info:
        if bus['ServiceNo'] ==data:
            ...
            delta1 = datetime.strptime(bus1time, '%Y-%m-%dT%H:%M:%S+08:00')-datetime.now()
            bus2time = bus['NextBus2']['EstimatedArrival']
            delta2 = datetime.strptime(bus2time, '%Y-%m-%dT%H:%M:%S+08:00') - datetime.now()
            bus3time = bus['NextBus3']['EstimatedArrival']
            delta3 = datetime.strptime(bus3time, '%Y-%m-%dT%H:%M:%S+08:00') - datetime.now()
            arrival_time = []
            for delta in [delta1,delta2,delta3]:
                minutes, seconds = divmod(delta, 60)
                arrival_time.append(minutes)
            context.bot.send_message(chat_id=update.effective_chat.id,text = 'The next bus is coming in {} minutes, second bus is in {} minutes and third bus is in {} minutes'.format(arrival_time[0],arrival_time[1],arrival_time[2]) )
