...
while value:
    busStopList += value
    skip += 500
    busStopUrl = 'http://datamall2.mytransport.sg/ltaodataservice/BusStops?$skip={}'.format(skip)
    print(busStopUrl)
    response = requests.get(busStopUrl, headers=headers)
...
def get_bus_stops(my_cord):
    ...
    for busStop in busStopList:
        ...
        distance = mpu.haversine_distance(my_cord, (busStop['Latitude'],busStop['Longitude']))
        if distance<0.3:
            result.append(busStop)
    return result

def get_bus_arrival(busStopCode):
    url = 'http://datamall2.mytransport.sg/ltaodataservice/BusArrivalv2?BusStopCode={}'.format(busStopCode)
